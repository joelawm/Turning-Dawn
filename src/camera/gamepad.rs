use bevy::{input::gamepad::{GamepadConnection::*, *}, prelude::*, window::PrimaryWindow};
use std::f32::consts::PI;
use super::{GamepadResource, ThirdPersonCamera};

pub struct GamePadPlugin;

impl Plugin for GamePadPlugin {
	fn build(&self, app: &mut App) {
		app.add_systems(Update, (connections, (orbit_gamepad, zoom_gamepad).run_if(resource_exists::<GamepadResource>)));
	}
}

fn connections(mut cmds: Commands, gamepad_res: Option<Res<GamepadResource>>, mut gamepad_evr: EventReader<GamepadConnectionEvent>) {
...
pub fn zoom_gamepad(btns: Res<ButtonInput<GamepadButton>>,gamepad_res: Option<Res<GamepadResource>>, mut cam_q: Query<&mut ThirdPersonCamera, With<ThirdPersonCamera>>) {
